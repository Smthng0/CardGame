...
public class AbilityMinionGeneratorTest {
    ...
    public void createMinions_OK() throws Exception {
        List<MinionCard> cardList = AbilityMinionGenerator.createMinions();

        assertTrue(cardList.size() == 5);
        assertTrue(cardList.get(0).getTitle().equals("Puddle Jumper"));
        assertTrue(cardList.get(1).getTitle().equals("Mantis"));
        assertTrue(cardList.get(2).getTitle().equals("Porcupine"));
        assertTrue(cardList.get(3).getTitle().equals("Puddle Jumpitor"));
        assertTrue(cardList.get(4).getTitle().equals("MantiPineJumper"));
    }
